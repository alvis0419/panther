...
#include "asm/../../panther/wlan/include/chip.h"
...
static void spi_panther_wait_event(u32 channel, u32 flags, compare_func compare)
{
    unsigned sr = 0;
    int interrupted;
    int done;
    ...
    interrupted = wait_event_interruptible(spi_data.waitq,
                                           (done = compare(sr = pdma_get_spi_srstat(&spi_data), flags)));
...
static void spi_panther_read_with_pdma(spi_cfg_t *p_priv, u32 data_len, u32 cmd_len,
                                    u8 *p_buf, u32 cmd_dtmp, u8 secure)
{
    ...
    spi_panther_wait_event(descriptor.channel, CH7_PDMA_ENABLE, any_bits_set);
#else
    pdma_pooling_wait();
#endif
}

static void spi_panther_write_with_pdma(spi_cfg_t *p_priv, u8 *p_cmd_buf, u32 data_len,
                                    u32 cmd_len, u8 *p_buf, u32 cmd_dtmp, u8 secure)
{
    ...
    // wait until PDMA finish work
#ifdef PDMA_INTERRUPT
    spi_panther_wait_event(descriptor.channel, CH6_PDMA_ENABLE, any_bits_set);
#else
    pdma_pooling_wait();
#endif
}
              ...
              spi_cmd_cfg_t *p_cmd_cfg, u8 *p_data_buf, u32 data_len, u8 secure, u32 dma_sel)
{
    ...
    if (dma_sel)
    {
        spi_panther_read_with_pdma(p_priv, data_len, cmd_len, p_data_buf, cmd_dtmp, secure);
    }
    else
#endif
    {
                  ...
                  spi_cmd_cfg_t *p_cmd_cfg, u8 *p_data_buf, u32 data_len, u8 secure, u32 dma_sel)
{
    ...
    if (dma_sel)
    {
        spi_panther_write_with_pdma(p_priv, p_cmd_buf, data_len, cmd_len, p_data_buf, cmd_dtmp, secure);
    }
    else
#endif
    {
